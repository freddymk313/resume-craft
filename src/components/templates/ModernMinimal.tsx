...
const ModernMinimal = ({ data }: Props) => {
  ...
  return (
    <div className="w-full bg-white text-gray-900 p-10 font-body" style={{ fontFamily: "var(--font-body)" }}>
      ...
      {summary && (
        <div className="mb-6">
          <h2 className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-2" style={{ fontFamily: "var(--font-display)" }}>Profile</h2>
            ...
            {experience.map((exp) => (
              <div key={exp.id}>
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-xs text-gray-400 whitespace-nowrap ml-4">{exp.startDate} — {exp.currentJob ? "Present" : exp.endDate}</span>
                </div>
                {exp.description && (
      ...
      {skills.length > 0 && (
        <div className="mb-6">
          <h2 className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-2" style={{ fontFamily: "var(--font-display)" }}>Skills</h2>
          <div className="flex flex-wrap gap-1.5">
            {skills.map((skill, i) => (
              <span key={i} className="text-xs px-2.5 py-1 rounded-full bg-gray-100 text-gray-600">{skill}</span>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-6">
        {languages.length > 0 && (
          <div>
            <h2 className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-2" style={{ fontFamily: "var(--font-display)" }}>Languages</h2>
            <p className="text-xs text-gray-600">{languages.join(", ")}</p>
          </div>
        )}
        {certifications.length > 0 && (
          <div>
            <h2 className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-2" style={{ fontFamily: "var(--font-display)" }}>Certifications</h2>
